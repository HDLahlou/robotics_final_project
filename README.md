...
We use `self.map.info.resolution` to calculate how many indices of the `self.truemap` array are equal to the length of a single grid square, `self.gridsize`. The function `node_to_occ` is basically a modified 2D-array-indices to 1d-array-index function. It takes the array indices `[i][j]` of a given maze cell and calculates the corresponding index in `self.truemap` with `self.gridsize`. Then, we can finally correctly identify which edges of a given maze cell are blocked by walls.  
